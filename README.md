...
- [x] 로그인 상태에서 헤더의 사용자id를 누를 경우 /mypage로 연결된다. 
  - 이 때 기존에 등록된 기본 이미지를 변경해서 업로드한다. 변경된 프로필 이미지는 메인 화면에서 볼 수 있어야 한다.

## 404 및 기타 에러 처리 페이지 구현
- [X] 존재하지 않는 URL 요청에 대해 404 페이지로 이동하도록 구현한다. 
- [x] 처리할 수 없는 메소드 요청에도 적절한 에러 페이지(405 등)으로 응답하도록 구현한다. 
...
## Javadoc 생성
- [x] 클래스와 퍼블릭 메소드에 javadoc 형식의 주석을 작성한다. 
- [x] 작성한 주석을 기반으로 javadoc을 생성한다. 
- [x] 생성된 웹페이지 문서를 GitHub Page 기능을 활용해서 업로드하고 주소를 댓글에 작성할 것!
