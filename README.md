...
#### 1) **핸들러 구현**
- **프로필 이미지**:
    - `ImageCreateHandler`: Base64 이미지를 받아 BLOB으로 저장.
    - `ImageDeleteHandler`: 기존 이미지를 삭제.
- **게시글 기능**:
    - `ArticleCreateHandler`: 게시글 내용을 받아 DB에 저장.
    - `ArticleReadHandler`: 특정 사용자의 최신 게시글 1건 조회.

#### 2) **UI 동작**
- **마이페이지**:
    - 프로필 이미지를 기본 `SVG`로 표시, 업로드된 이미지는 동그라미 영역에 맞게 출력.
    - 이미지 삭제 버튼으로 프로필 이미지를 초기화.
- **메인 페이지**:
    - 게시글 조회 시 이미지와 내용 표시, 없을 경우 기본 안내.

#### 3) **메인 화면 수정 (`index.html`)**
- **DOMContentLoaded 시** `/api/article` 요청  
  → **게시글**(내용·이미지)을 `<img class="post__img" />`와 `<p class="post__article">`에 표시.
- 이미지가 없으면 `<img>`에 `src`를 제거, 내용이 없으면 기본 문구(또는 빈 값)로 대체.
...
#### 1) **이미지 처리 효율성**
- Base64 인코딩/디코딩의 데이터 크기 증가 및 메모리 사용 문제.
- 클라이언트-서버 간 이미지 데이터를 전송하는 방법 개선 필요성.
- **Base64 인코딩/디코딩**:
    - 업로드 시: 브라우저에서 파일 → Base64 변환 후 서버 전송
    - DB 저장 시: Base64 → `byte[]`(BLOB)
    - 재조회 시: `byte[]` → Base64 → `<img>` 태그
- DB·메모리 사용량 고려 필요.
