...
public class MyTemplateEngine implements TemplateEngine {
    ...
    public String render(String template, Map<String, Object> model) {
        ...
        for (int i = 0; i < matchingResults.size(); i++) {
            ...
            rendered.append(tagRenderer.handle(model, result.tagAttributes, childrenTemplate));
        }
    ...
    private String extractChildrenTemplate(String template, TagMatchingResult result) {
        int openTagEnd = template.indexOf(OPEN_TAG_SUFFIX, result.firstTagStart) + 1;
        int closeTagStart = template.lastIndexOf(CLOSE_TAG_PREFIX + result.tagName, result.lastTagEnd);
        ...
        while (matcher.find()) {
            ...
            if (tagName.startsWith(CLOSE_TAG_NAME_PREFIX)) {
                ...
                if (openTagStack.isEmpty()) {
                    ...
                    matchingResults.add(new TagMatchingResult(currentTagStart, currentTagEnd, currentTagName, currentTagAttributes));
    ...
    private Map<String, String> parseAttributes(String attributes) {
        ...
        String[] split = attributes.split(ATTRIBUTE_DELIMITER);
        for (String s : split) {
            if (s.isBlank()) {
                continue;
            }
            String[] keyValue = s.split(KEY_VALUE_DELIMITER);
